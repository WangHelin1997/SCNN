...
class WaveMsNet(nn.Module):
    def __init__(self):
        ...
        self.conv3_1 = nn.Conv1d(in_channels=32, out_channels=32, kernel_size=5, stride=1, padding=5)
        self.conv3_2 = nn.Conv1d(in_channels=32, out_channels=32, kernel_size=5, stride=1, padding=5)
        self.conv3_3 = nn.Conv1d(in_channels=32, out_channels=32, kernel_size=5, stride=1, padding=5)

        self.bn3_1 = nn.BatchNorm1d(32)
        self.bn3_2 = nn.BatchNorm1d(32)
        self.bn3_3 = nn.BatchNorm1d(32)

        self.pool3_1 = nn.MaxPool1d(kernel_size=10, stride=10)
        self.pool3_2 = nn.MaxPool1d(kernel_size=2, stride=2)
        self.pool3_3 = nn.MaxPool1d(kernel_size=1, stride=1)

        #add layer
        self.conv4_1 = nn.Conv1d(in_channels=32, out_channels=32, kernel_size=5, stride=1, padding=5)
        self.conv4_2 = nn.Conv1d(in_channels=32, out_channels=32, kernel_size=5, stride=1, padding=5)
        self.conv4_3 = nn.Conv1d(in_channels=32, out_channels=32, kernel_size=5, stride=1, padding=5)

        self.bn4_1 = nn.BatchNorm1d(32)
        self.bn4_2 = nn.BatchNorm1d(32)
        self.bn4_3 = nn.BatchNorm1d(32)

        self.pool4_1 = nn.MaxPool1d(kernel_size=15, stride=15)
        self.pool4_2 = nn.MaxPool1d(kernel_size=15, stride=15)
        self.pool4_3 = nn.MaxPool1d(kernel_size=15, stride=15)
        ...
        self.pool5 = nn.MaxPool2d(kernel_size=(1, 1), stride=(1, 1))
        ...
        self.pool7 = nn.MaxPool2d(kernel_size=(1, 1), stride=(1, 1))
    ...
    def forward(self, x):
        ...
        x1 = self.pool3_1(x1)
        x2 = self.pool3_2(x2)
        x3 = self.pool3_3(x3)  # (batchSize, 32L, 441L)

        x1 = self.relu(self.bn4_1(self.conv4_1(x1)))
        x2 = self.relu(self.bn4_2(self.conv4_2(x2)))
        x3 = self.relu(self.bn4_3(self.conv4_3(x3)))

        x1 = self.pool4_1(x1)
        x2 = self.pool4_2(x2)
        x3 = self.pool4_3(x3)  # (batchSize, 32L, 441L)
        ...
        h = self.bn7(h)
        h = self.relu(h)
        h = self.pool7(h)  # (bs, 256L, 4L, 5L)

        h = self.conv8(h)
        h = self.bn8(h)
        h = self.relu(h)
        h = self.pool8(h)  # (bs, 256L, 4L, 5L)
...
class WaveMsNet_fixed_logmel(nn.Module):
    def __init__(self, phase):
        super(WaveMsNet_fixed_logmel, self).__init__()
        self.phase = phase
        self.conv1_1 = nn.Conv1d(in_channels=1, out_channels=32, kernel_size=11, stride=1, padding=5)
        self.conv1_2 = nn.Conv1d(in_channels=1, out_channels=32, kernel_size=51, stride=5, padding=25)
        self.conv1_3 = nn.Conv1d(in_channels=1, out_channels=32, kernel_size=101, stride=10, padding=50)
    ...
    def forward(self, x, feats=None):
        # input: (batchSize, 1L, 66150L)
        print(x.shape)
        print(self.conv1_1(x).shape)
        print(self.conv1_2(x).shape)
        print(self.conv1_3(x).shape)
        x1 = self.relu(self.bn1_1(self.conv1_1(x)))
        x2 = self.relu(self.bn1_2(self.conv1_2(x)))
        x3 = self.relu(self.bn1_3(self.conv1_3(x)))
