...
parser.add_argument('--mode', type=str, default='train',
                            help='train or test')
parser.add_argument('--model', type=str, default='../model_DCASE2018/WaveMsNet_fold0_v2_epoch400.pkl',
...
parser.add_argument('--test_slices_interval', type=int, default=0.2,
                            help='slices number of one record divide into.')
parser.add_argument('--output', type=str, default = '../output_DCASE2018/output_second_phase.txt',
                            help='output file.')
...
def train(model, optimizer, train_loader, epoch, phase):
#{{{
    model.train()
    start = time.time()
...
def test(model, test_pkl, phase, epoch):
#{{{
    model.eval()
    ...
    if phase == 1:
        ...
        writer.add_scalar('scalar/Test_Acc', test_acc, epoch)
...
def main_on_fold(foldNum, trainPkl, validPkl):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        auc = train(model_WNN, optimizer_WNN, train_loader, epoch, args.phase)
        #  test and save the best model
        # if epoch % args.model_save_interval == 0:
        if auc > 0 :
            if epoch % args.model_save_interval == 0:
                test_acc = test(model_WNN, validPkl, args.phase, epoch)
            ...
            #
            #     model_name = '../model/' + args.network + '_phase'+ str(args.phase)+ '_epoch' + str(epoch) + '.pkl'
            #     best_model_name = '../model/' + args.network + '_phase'+ str(args.phase)+ '_best.pkl'
...
def main():
    print(args.network)
    print(args.phase)
    trainPkl = '../data_wave_DCASE2018/fold1_train.m'
    validPkl = '../data_wave_DCASE2018/fold1_test.m'
    start = time.time()
    main_on_fold(1 , trainPkl, validPkl)
